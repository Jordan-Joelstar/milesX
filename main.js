   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2349054039891")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +2349054039891 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ 𝗠⃪𝗔⃪𝗝⃪𝗜⃪𝗡⃪ 𝗠⃪𝗜⃪𝗟⃪𝗘⃪𝗦⃪ ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: `))
            console.log(chalk.magenta(`${themeemoji} GITHUB: JORDAN-JOELSTAR`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 2349054039891`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: JORDAN-JOELSTAR\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
